...
class ItineraryController {
    ...
    def convertToJSON(it1){
        ...
        def root = builder.itinerary {
            ...
            name it1.name
            days(
                it1.days.collect{ Day d ->
                    [day:d.day, dayDate:d.dayDate, events: events(
                        d.events.collect{ Event e ->
                            [name:e.name,
                                ...
                                e.operations.collect{ OperationTime o ->
                                    [startMonth:o.startMonth, endMonth:o.endMonth, hours:hours(
                                        o.hours.collect{ Hours h ->
    ...
    def getResultsInHTML(searchResults) {
        ...
        for(event in searchResults) {
            ...
            newtag += "</div>\n" +
                "<div>Adult Price |\n"
            if (event.pricing.adultPrice == -2 || event.pricing.adultPrice == 0 || event.pricing.childPrice == -1) {
                newtag += "Free\n"
            } else if (event.pricing.adultPrice == -3) {
                newtag += "\$"
            } else if (event.pricing.adultPrice == -4) {
                newtag += "\$\$"
            } else if (event.pricing.adultPrice == -5) {
                newtag += "\$\$\$"
            } else {
            ...
            newtag += "</div>\n" +
                "<div>Child Price |\n"
            if (event.pricing.childPrice == -2 || event.pricing.childPrice == 0 || event.pricing.childPrice == -1) {
                newtag += "Free\n"
            } else if (event.pricing.childPrice == -3) {
                newtag += "\$"
            } else if (event.pricing.childPrice == -4) {
                newtag += "\$\$"
            } else if (event.pricing.childPrice == -5) {
                newtag += "\$\$\$"
            } else {
