        ...
        if (lineNo != 0 && !columns[1].trim().isEmpty()) {
            ...
            if (!columns[1].equals("^")) {
                ...
                if (event != null) {
                    ...
                    event.description = columns[2]
                    println event.description
                    // Add the picture
                    // Check that this picture isn't already here
                    if (event.picturePaths == null || !event.picturePaths.contains(columns[51]))
                        event.addToPicturePaths(columns[51])
                    println event.picturePaths
                    // Add the picture source
                    if (event.pictureSources == null || !event.pictureSources.contains(columns[52]))
                        event.addToPictureSources(columns[52])
                    println event.pictureSources
                    event.save()
                } else
